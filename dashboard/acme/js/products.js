'use strict' 
 ...
 catList.addEventListener("change", function () { 
  ...
  let catIdURL = "/dashboard/acme/products/index.php?action=getInventoryItems&categoryId=" + categoryId; 
  fetch(catIdURL) 
...
function buildProductList(data) { 
 ...
 data.forEach(function (element) { 
  ...
  dataTable += `<td><a href='/dashboard/acme/products?action=mod&id=${element.invId}' title='Click to modify'>Modify</a></td>`; 
  dataTable += `<td><a href='/dashboard/acme/products?action=del&id=${element.invId}' title='Click to delete'>Delete</a></td></tr>`; 
console.log(element.invId);
}) 
 dataTable += '</tbody>'; 
